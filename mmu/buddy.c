...
static UCHAR l_ubaMemPool[ONPS_BUDDY_MEM_SIZE];
static ST_BUDDY_AREA l_staArea[ONPS_BUDDY_ARER_COUNT];
static ST_BUDDY_PAGE l_staPage[ONPS_BUDDY_MEM_SIZE / ONPS_BUDDY_PAGE_SIZE + 1];
#if 0
static STCB_BUDDY_PAGE_NODE l_stcbaFreePage[ONPS_BUDDY_MEM_SIZE / ONPS_BUDDY_PAGE_SIZE + 1];
#else
...
{
    INT i;
    UINT unPageSize = ONPS_BUDDY_PAGE_SIZE;

#if 0
    //* 存储页面控制信息的链表必须先初始化，接下来就要用到
    for (i = 0; i < (INT)(ONPS_BUDDY_MEM_SIZE / ONPS_BUDDY_PAGE_SIZE + 1); i++)
    {
    ...
    l_staArea[ONPS_BUDDY_ARER_COUNT - 1].pstFreed = pstPage;

    //* 计算并存储各页块管理单元的单个页面大小
    l_staArea[0].unPageSize = unPageSize;
    for (i = 1; i < ONPS_BUDDY_ARER_COUNT; i++)
    {
        unPageSize *= 2;
...
void *buddy_alloc(UINT unSize, EN_ONPSERR *penErr)
{
    ...
    //* 确定页块索引
    for (i = 0; i < ONPS_BUDDY_ARER_COUNT; i++)
    {
...
FLOAT buddy_usage(void)
{
    ...
    return (FLOAT)unUsedPageSize / (FLOAT)ONPS_BUDDY_MEM_SIZE;
}
...
FLOAT buddy_usage_details(UINT *punTotalBytes, UINT *punUsedBytes, UINT *punMaxFreedPageSize, UINT *punMinFreedPageSize)
{
    INT i;
    UINT unUsedPageSize = 0;
    UINT unMaxFreedPageSize = ONPS_BUDDY_PAGE_SIZE, unMinFreedPageSize = ONPS_BUDDY_PAGE_SIZE;

    *punTotalBytes = ONPS_BUDDY_MEM_SIZE;
    ...
    *punUsedBytes = unUsedPageSize;
    ...
    if (unUsedPageSize < ONPS_BUDDY_MEM_SIZE)
    {
...
#endif //* #ifdef ONPS_ENABLE_BUDDY_MMU
