...
PST_NETIF netif_get_by_ip(UINT unNetifIp, BOOL blIsForSending)
{
    os_thread_mutex_lock(l_hMtxNetif);
    {
        ...
        while (pstNextNode)
        {
        ...
        #if defined(ONPS_ENABLE_ETHERNET) && defined(ONPS_ENABLE_ETH_EXTRA_IP)
            ...
            if (NIF_ETHERNET == pstNextNode->stIf.enType)
            {
                ...
                CHAR i;
                for (i = 0; i < ONPS_ETH_EXTRA_IP_NUM; i++)
                {
        ...
        #endif //* #if defined(ONPS_ENABLE_ETHERNET) && defined(ONPS_ENABLE_ETH_EXTRA_IP)
    ...
    if (unNextIp)
    {
        ...
        for (i = 0; i < ONPS_ETH_EXTRA_IP_NUM; i++)
        {
            if (pstExtra->staExtraIp[i].unAddr)
            {
                ...
                {
                    i++;
                    if (i < ONPS_ETH_EXTRA_IP_NUM && pstExtra->staExtraIp[i].unAddr)
                    {
