...
#if defined(ONPS_ENABLE_NETTOOLS_DNS_CLIENT) && defined(ONPS_ENABLE_NVTCMD_NSLOOKUP)
#include "net_tools/dns.h"
#endif //* #if defined(ONPS_ENABLE_NETTOOLS_DNS_CLIENT) && defined(ONPS_ENABLE_NVTCMD_NSLOOKUP)
...
#ifdef ONPS_ENABLE_NVTCMD_MEMUSAGE
static INT mem_usage(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle);
#endif //* #ifdef ONPS_ENABLE_NVTCMD_MEMUSAGE

#ifdef ONPS_ENABLE_NVTCMD_NETIF
static INT netif(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle);
#endif //* #ifdef ONPS_ENABLE_NVTCMD_NETIF

#if defined(ONPS_ENABLE_NVTCMD_IFIP) && defined(ONPS_ENABLE_ETHERNET)
static INT ifip(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle);
#endif //* #if defined(ONPS_ENABLE_NVTCMD_IFIP) && defined(ONPS_ENABLE_ETHERNET)
...
static void nvt_cmd_cache_add(PSTCB_NVT pstcbNvt)
{
    ...
    if (nMovedIdx < 0)
    {
        ...
        INT nRemainSpaceSize = ONPS_NVTCMDCACHE_SIZE - nCacheCmdLen;
        if (nRemainSpaceSize < nCmdLen) //* 如果不够存储，删除最老的指令
        {
            ...
            for (i = 0; i < ONPS_NVTCMDCACHE_SIZE; )
            {
                ...
                else
                {
                    ...
                    if (nCacheCmdLen < ONPS_NVTCMDCACHE_SIZE)
...
#ifdef ONPS_ENABLE_NVTCMDCACHE
    EN_ONPSERR enErr;
    pstcbNvt->pszCmdCache = (CHAR *)buddy_alloc(ONPS_NVTCMDCACHE_SIZE, &enErr);
    if (pstcbNvt->pszCmdCache)
    {
        pstcbNvt->bCmdNum = 0;
        pstcbNvt->bCmdIdx = 0;
        memset(pstcbNvt->pszCmdCache, 0, ONPS_NVTCMDCACHE_SIZE);
    }
    else
...
#define NVTHELP_NTPDATE_DISDNS  "Due to the fact that the protocol stack configuration item ONPS_ENABLE_NETTOOLS_DNS_CLIENT is not set, DNS query functionality is not supported. Please use the IP address directly.\r\n"
static INT ntpdate(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle)
{
    PSTCB_TELNETCLT pstTelnetClt = (PSTCB_TELNETCLT)((UCHAR *)ullNvtHandle - offsetof(STCB_TELNETCLT, stcbNvt));
    if (argc == 2 || argc == 3)
