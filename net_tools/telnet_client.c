...
void telnet_clt_entry(void *pvParam)
{
    ...
    {
        EN_ONPSERR enErr;
        UCHAR *pubRcvBuf = (UCHAR *)buddy_alloc(ONPS_TELNETCLT_RCVBUF_SIZE, &enErr);
        if (pubRcvBuf)
        {
            //* 非阻塞
            ...
            {
                nRcvBytes = nvt_input(stArgs.ullNvtHandle, pubRcvBuf, ONPS_TELNETCLT_RCVBUF_SIZE);
                if (nRcvBytes)
                {
                ...
                nRcvBytes = recv(hTelSocket, pubRcvBuf, ONPS_TELNETCLT_RCVBUF_SIZE);
                if (nRcvBytes > 0)
                {
                    blIsNotRcvedData = FALSE;
                    usZeroNum = 0;
