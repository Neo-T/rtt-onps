...
void thread_one_shot_timer_count(void *pvParam)
{
    ...
    UINT unDelayAckTimeout = ONPS_TCP_ACK_DELAY_MSECS < 40 || ONPS_TCP_ACK_DELAY_MSECS > 200 ? 100 - 1 : ONPS_TCP_ACK_DELAY_MSECS - 1;
#endif
                ...
                if (pstcbTcpSndTimer)
                {
                    ...
                    if (os_get_system_msecs() - pstcbTcpSndTimer->unSendMSecs > (UINT)pstcbTcpSndTimer->usRto)
                    {
                        if (pstcbTcpSndTimer->bIsNotSacked)
                        {
                            ...
                            if (pubData)
                            {
                                UINT unStartReadIdx = pstcbTcpSndTimer->unLeft % ONPS_TCPSNDBUF_SIZE;
                                UINT unEndReadIdx = pstcbTcpSndTimer->unRight % ONPS_TCPSNDBUF_SIZE;
