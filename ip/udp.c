...
#if defined(ONPS_ENABLE_IPV6) && defined(ONPS_ENABLE_ETHERNET)
...
#endif
{
    ...
    INT nRtnVal;
    if (AF_INET == bFamily)
        nRtnVal = ip_send_ext(*(in_addr_t *)pvSrcAddr, *(in_addr_t *)pvDstAddr, UDP, ONPS_IP_TTL_DEFAULT, sBufListHead, penErr);
    else
    {
...
#else
    INT nRtnVal = ip_send_ext(unSrcAddr, unDstAddr, UDP, ONPS_IP_TTL_DEFAULT, sBufListHead, penErr);
#endif
...
INT udp_send_ext(INT nInput, SHORT sBufListHead, in_addr_t unDstIp, USHORT usDstPort, in_addr_t unSrcIp, PST_NETIF pstNetif, EN_ONPSERR *penErr)
{
    ...
    //* 发送之
    INT nRtnVal = ip_send(pstNetif, NULL, unSrcIp, unDstIp, UDP, ONPS_IP_TTL_DEFAULT, sBufListHead, penErr);
...
void ipv6_udp_recv(PST_NETIF pstNetif, UCHAR ubaSrcAddr[16], UCHAR ubaDstAddr[16], UCHAR *pubPacket, INT nPacketLen)
{
    ...
    if (pstLink)
    {
        ...
        {
    #if defined(ONPS_ENABLE_PRINTF) && defined(ONPS_DEBUG_LEVEL)// > 3
            CHAR szIpv6[40];
