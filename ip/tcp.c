...
#endif
{
    ...
    //* 发送之
    INT nRtnVal = ip_send_ext(unSrcAddr, unDstAddr, TCP, ONPS_IP_TTL_DEFAULT, sBufListHead, penErr);
#ifdef ONPS_ENABLE_SACK
    if (!blIsSpecSeqNum)
...
INT tcp_send_syn(INT nInput, in_addr_t unSrvAddr, USHORT usSrvPort, int nConnTimeout)
{
    ...
    pstLink->stcbWaitAck.pstTimer = one_shot_timer_new(tcp_ack_timeout_handler, pstLink, nConnTimeout ? nConnTimeout : ONPS_TCP_CONN_TIMEOUT);
...
#ifndef ONPS_ENABLE_SACK
    pstLink->stcbWaitAck.bIsAcked = FALSE;
    pstLink->stcbWaitAck.pstTimer = one_shot_timer_new(tcp_ack_timeout_handler, pstLink, nWaitAckTimeout ? nWaitAckTimeout : ONPS_TCP_ACK_TIMEOUT);
    if (!pstLink->stcbWaitAck.pstTimer)
    {
        onps_set_last_error(nInput, ERRNOIDLETIMER);
        return -1;
...
static void tcp_send_timer_retransmit(PST_TCPLINK pstLink, PSTCB_TCPSENDTIMER pstcbSndTimer)
{
    ...
    if (pubData)
    {
        UINT unStartReadIdx = pstcbSndTimer->unLeft % ONPS_TCPSNDBUF_SIZE;
        UINT unEndReadIdx = pstcbSndTimer->unRight % ONPS_TCPSNDBUF_SIZE;
        if (unEndReadIdx > unStartReadIdx)
            memcpy(pubData, pstLink->stcbSend.pubSndBuf + unStartReadIdx, unEndReadIdx - unStartReadIdx);
        else
        {
            UINT unCpyBytes = ONPS_TCPSNDBUF_SIZE - unStartReadIdx;
...
void tcp_recv(void *pvSrcAddr, void *pvDstAddr, UCHAR *pubPacket, INT nPacketLen, EN_NPSPROTOCOL enProtocol)
{
    ...
    {
#if defined(ONPS_ENABLE_PRINTF) && ONPS_DEBUG_LEVEL > 3
    ...
    if (uint_before(unStartSeqNum, pstLink->stcbSend.unWriteBytes))
    {
        ...
        if (!nPeerWndSize)
        {
            if(pstLink->stcbSend.bIsZeroWnd)
            {
                ...
                {
                    tcp_send_data_ext(pstLink->stcbWaitAck.nInput, pstLink->stcbSend.pubSndBuf + (unStartSeqNum % ONPS_TCPSNDBUF_SIZE), 1, unStartSeqNum + 1);
        ...
        if (NULL != pstcbSndTimer)
        {
            ...
            if (pubData)
            {
                ...
                {
                    UINT unFirstCpyBytes = ONPS_TCPSNDBUF_SIZE - unStartReadIdx;
