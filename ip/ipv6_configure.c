...
static ST_IPv6_DYNADDR l_staIpv6DynAddrs[ONPS_IPV6_CFG_ADDR_NUM]; //* ipv6动态地址信息存储单元
static ST_IPv6_ROUTER l_staIpv6Routers[ONPS_IPV6_ROUTER_NUM];     //* ipv6路由器信息存储单元
...
{
    array_linked_list_put(pstDynAddrNode, &l_bFreeIpv6DynAddrList, l_staIpv6DynAddrs, (UCHAR)sizeof(ST_IPv6_DYNADDR), ONPS_IPV6_CFG_ADDR_NUM, offsetof(ST_IPv6_DYNADDR, bNextAddr));
}
...
{
    array_linked_list_put(pstRouterNode, &l_bFreeIpv6RouterList, l_staIpv6Routers, (UCHAR)sizeof(ST_IPv6_ROUTER), ONPS_IPV6_ROUTER_NUM, offsetof(ST_IPv6_ROUTER, bNextRouter));
}

PST_IPv6_DYNADDR ipv6_dyn_addr_get(CHAR bDynAddr)
{
    if (bDynAddr >= 0 && bDynAddr < ONPS_IPV6_CFG_ADDR_NUM)
...
{
    if (bRouter >= 0 && bRouter < ONPS_IPV6_ROUTER_NUM)
...
{
    return array_linked_list_get_index(pstRouter, l_staIpv6Routers, (UCHAR)sizeof(ST_IPv6_ROUTER), ONPS_IPV6_ROUTER_NUM);
}

void netif_ipv6_dyn_addr_add(PST_NETIF pstNetif, PST_IPv6_DYNADDR pstDynAddr)
{
    array_linked_list_put_tail_safe(pstDynAddr, &pstNetif->stIPv6.bDynAddr, l_staIpv6DynAddrs, (UCHAR)sizeof(ST_IPv6_DYNADDR), ONPS_IPV6_CFG_ADDR_NUM, offsetof(ST_IPv6_DYNADDR, bNextAddr));
}
...
{
    array_linked_list_put_tail_safe(pstRouter, &pstNetif->stIPv6.bRouter, l_staIpv6Routers, (UCHAR)sizeof(ST_IPv6_ROUTER), ONPS_IPV6_ROUTER_NUM, offsetof(ST_IPv6_ROUTER, bNextRouter));
}
...
#if defined(ONPS_ENABLE_PRINTF) && ONPS_DEBUG_LEVEL > 1
//* 输出ipv6配置信息
