            ...
            PFUN_PINGERRHANDLER pfunErrHandler, UCHAR ubWaitSecs, void *pvParam, EN_ONPSERR *penErr)
{
    ...
    if (AF_INET == bFamily)
    {
        ...
        if (nRcvedBytes > 0)
        {
            ...
            pfunRcvHandler(nPing + 1, &unFromAddr, usReplySeqNum, ubaEchoData, (UCHAR)nRcvedBytes, ubReplyTTL, (UCHAR)(pfunGetCurMSecs ? pfunGetCurMSecs() - unStartMillisecs : 0), pvParam);
        }
    }
#ifdef ONPS_ENABLE_IPV6
    else
...
#if defined(ONPS_ENABLE_NETTOOLS_TELNETSRV) && defined(ONPS_ENABLE_NVTCMD_PING)
...
void nvt_cmd_ping_entry(void *pvParam)
{
    ...
    //* RISC-V
    //nvt_outputf(stcbPing.stArgs.ullNvtHandle, 256, "%s ping statistics:\r\n%d packets transimitted, %d received, time %d ms\r\nrtt min/max = %d/%d ms\r\n",
    //                    stcbPing.stArgs.szDstIp, unSeqNum, unRecvedPackets, stcbPing.stStatistics.unRttTotal,
    //                    stcbPing.stStatistics.unRttMin, stcbPing.stStatistics.unRttMax);
...
#endif //* #if defined(ONPS_ENABLE_NETTOOLS_TELNETSRV) && defined(ONPS_ENABLE_NVTCMD_PING)
