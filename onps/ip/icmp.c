...
static void icmp_send_echo_reply(PST_NETIF pstNetif, UCHAR *pubDstMacAddr, UCHAR *pubPacket, INT nPacketLen)
{
    ...
    //* 发送数据
    if (icmp_send(pstNetif, pubDstMacAddr, pstReqIpHdr->unDstIP, pstReqIpHdr->unSrcIP, ICMP_ECHOREPLY, 0, ONPS_IP_TTL_DEFAULT, sBufListHead, &enErr) < 0)
    {
#if defined(ONPS_ENABLE_PRINTF) && defined(ONPS_DEBUG_LEVEL)
        printf("icmp_send_echo_reply() failed, %s\r\n", onps_error(enErr));
#endif
...
#if defined(ONPS_ENABLE_PRINTF) && ONPS_DEBUG_LEVEL > 2
