...
static STCB_ETHIPv6MAC l_stcbaEthIpv6Mac[ONPS_ETHERNET_NUM];
...
void ipv6_mac_add_entry(PST_NETIF pstNetif, UCHAR ubaIpv6[16], UCHAR ubaMacAddr[ETH_MAC_ADDR_LEN], BOOL blIsOverride)
{
    ...
    INT i;
    for (i = 0; i < pstcbIpv6Mac->bEntriesNum/*ONPS_IPV6TOMAC_ENTRY_NUM*/; i++)
    {
    ...
    if (i < ONPS_IPV6TOMAC_ENTRY_NUM)
        pstcbIpv6Mac->bEntriesNum++;
    else
    {
        INT nFirstEntry = 0;
        for (i = 1; i < ONPS_IPV6TOMAC_ENTRY_NUM; i++)
        {
...
static void icmpv6_ra_handler(PST_NETIF pstNetif, UCHAR ubaRouterIpv6[16], UCHAR *pubIcmpv6, USHORT usIcmpv6PktLen)
{
    ...
    else
    {
        ...
        if (!pstRouter)
        {
    #if defined(ONPS_ENABLE_PRINTF) && ONPS_DEBUG_LEVEL > 0
