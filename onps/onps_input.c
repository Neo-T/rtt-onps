...
static STCB_ONPS_INPUT l_stcbaInput[ONPS_SOCKET_NUM_MAX];
static ST_SLINKEDLIST_NODE l_staNodes[ONPS_SOCKET_NUM_MAX];
...
BOOL onps_input_init(EN_ONPSERR *penErr)
{
    *penErr = ERRNO;
    ...
    for (i = 1; i < ONPS_SOCKET_NUM_MAX; i++)
    {
...
{
    INT i;
    for (i = 0; i < ONPS_SOCKET_NUM_MAX; i++)
    {
...
#endif
{
    ...
    case IPPROTO_ICMP:
    case IPPROTO_ICMPv6:
        unSize = ONPS_ICMPRCVBUF_SIZE;
        break;

    case IPPROTO_TCP:
        unSize = ONPS_TCPRCVBUF_SIZE;
        break;
...
{
    if (nInputSrv < 0 || nInputSrv > ONPS_SOCKET_NUM_MAX - 1)
    {
        if (penErr)
    ...
    UINT unSize = ONPS_TCPRCVBUF_SIZE;
...
{
    if (nInput < 0 || nInput > ONPS_SOCKET_NUM_MAX - 1)
    {
...
void onps_input_sem_post_tcpsrv_accept(INT nSrvInput, INT nCltInput, UINT unLocalSeqNum)
{
    ...
    if (nSrvInput < 0 || nSrvInput > ONPS_SOCKET_NUM_MAX - 1 || nCltInput < 0 || nCltInput > ONPS_SOCKET_NUM_MAX - 1)
        goto __lblErr;
...
__lblPortNew:
    usPort = 65535 - (USHORT)(rand() % (ONPS_TCPUDP_PORT_START + 1));
    ...
    if (IPPROTO_TCP == (EN_IPPROTO)pstcbInput->ubIPProto)
    {
        ...
        if (pstcbInput->pvAttach)
        {
            ...
            if (pstLink->bState == TLSCONNECTED)
            {
                os_enter_critical();
                {
                    UINT unAckIdx = (pstLink->stLocal.unSeqNum - 1) % ONPS_TCPSNDBUF_SIZE;
                    UINT unWriteIdx = pstLink->stcbSend.unWriteBytes % ONPS_TCPSNDBUF_SIZE;
                    if (unWriteIdx >= unAckIdx)
                    {
                        if (!(unWriteIdx == unAckIdx && pstLink->stcbSend.unWriteBytes != pstLink->stLocal.unSeqNum - 1))
                        {
                            UINT unRemainBytes = ONPS_TCPSNDBUF_SIZE - unWriteIdx;
